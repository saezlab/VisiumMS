...
def get_corrs_slide(sample_id):
    ...
    cell_types = props.columns.values

    # Init empty mats
    corrs = np.zeros((cell_types.size, cell_types.size))
    pvals = np.zeros((cell_types.size, cell_types.size))
    for i, c_a in enumerate(cell_types):
        for j, c_b in enumerate(cell_types):
            if c_a == c_b:
                corrs[i, j] = np.nan
                pvals[i, j] = np.nan
            else:
                ...
                # Compute pearson
                corrs[i, j], pvals[i, j] = stats.pearsonr(props[c_a].values, props[c_b].values)

    # Transform to dfs
    corrs = pd.DataFrame(corrs, index=cell_types, columns=cell_types)
    pvals = pd.DataFrame(pvals, index=cell_types, columns=cell_types)
...
def plot_props_corrs(lesion_type):
    # Read meta
    ...
    for sample_id in meta.index.values:
        corr, pval = get_corrs_slide(sample_id)
        corrs.append(corr)
        pvals.append(pval)
    ...
    # Define color map
    #cmap = matplotlib.colors.LinearSegmentedColormap.from_list("", ["white","white","#8da0cb"])
    cmap = plt.get_cmap('coolwarm').copy()
    cmap.set_bad(color='gray')

    fig, ax = plt.subplots(1, 1, figsize=(4, 4), facecolor='white', dpi=125)
    htm = sns.heatmap(corrs, cmap=cmap, square=True, center=0, vmax=1, vmin=-1, ax=ax, cbar_kws={"shrink": .5, "aspect": 5},
    ...
    ax.set_title('Correlation {0}'.format(lesion_type), loc='left')
    os.makedirs('figures/corr_prop_prop/', exist_ok=True)
    fig.savefig('figures/corr_prop_prop/{0}.pdf'.format(lesion_type.replace(' ', '')), bbox_inches='tight')

plot_props_corrs('Chronic Active')
plot_props_corrs('Control')
