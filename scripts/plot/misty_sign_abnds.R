...
plot_misty_res <- function(lesion_type, sign_type){
    ...
    res_dir <- sprintf("figures/misty_signs_abnds/%s", lesion_type)
    dir.create(res_dir, showWarnings = FALSE, recursive = TRUE)

    # Plot gain R2
    plot_path <- sprintf("%s/%s_gain_R2.pdf", res_dir, sign_type)
    pdf(file = plot_path, width = 4, height = 4)
    plot_improvement_stats(res, "gain.R2")
    dev.off()
