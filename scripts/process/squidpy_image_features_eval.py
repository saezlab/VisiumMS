...
# python scripts/process/squidpy_image_features_eval.py
...
input_dir = current_folder / ".." / ".." / "data" / "prc" / "images" / "squidply_image_features"
output_dir = current_folder / ".." / ".." / "out" / "squidpy_image_features_eval"
...
for feature in ["texture", "summary", "histogram"]:
    mean_image_features = np.array([adata.obsm[feature].mean(axis=0) for adata in adata_dict.values()])
    ...
    fig.savefig(output_dir / ("pca_mean_image_features_" + feature + ".png"), dpi=300)
    plt.close()
    ### ------------------------------- ###

    ### 2. cluster cells based on image feature space and compute fractions of clusters per image ###
    image_features_mtx = np.vstack([adata.obsm[feature] for adata in adata_dict.values()])
    ...
    fig.savefig(output_dir / ("ht_clustered_image_features_" + feature + ".png"), dpi=300)
    plt.close()
    ...
    plt.legend(bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.)
    fig.savefig(output_dir / ("pca_clustered_image_features_" + feature + ".png"), dpi=300)
    plt.close()
    ### ------------------------------- ###