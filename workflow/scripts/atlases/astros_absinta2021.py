...
for group_a in groups_a:
    for group_b in groups_b:
        genes_a = deg_abs[deg_abs['group'] == group_a].set_index('names')
        genes_b = deg_ler[deg_ler['group'] == group_b].set_index('names')
        inter = np.intersect1d(genes_a.index, genes_b.index)
        union = np.union1d(genes_a.index, genes_b.index)
        jacc = inter.size / union.size
        df.append([group_a, group_b, inter.size, union.size, jacc])
df = pd.DataFrame(df, columns=['group_a', 'group_b', 'inter', 'union', 'jacc'])
df = df.pivot(index='group_a', columns='group_b', values='jacc')

# Plot
order = df.max(0).sort_values(ascending=False).index
fig1, ax = plt.subplots(1, 1, figsize=(6, 3), dpi=150)
sns.heatmap(df.loc[:, order], cmap='Blues', square=True, cbar_kws={"shrink": 0.25, 'aspect': 5, 'label': 'Jaccard index'}, ax=ax)
