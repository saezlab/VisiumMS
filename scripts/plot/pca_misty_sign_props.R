...
plot_misty_importances <- function(lesions, view_name, sign_type){

    # Read PCS and loadings
    lesions <- stringr::str_c(stringr::str_replace_all(lesions, " ", ""), collapse='|')
    pcs <- read.csv(sprintf("data/prc/pca_misty/sign_props/pcs_%s_%s_%s.csv",
                            lesions, sign_type, view_name))
    ldns <- read.csv(sprintf("data/prc/pca_misty/sign_props/ldns_%s_%s_%s.csv",
    ...
    # Plot
    lesions <- stringr::str_c(stringr::str_replace_all(lesions, " ", ""), collapse='|')
    dir.create("figures/pca_misty_sign_props/", showWarnings = FALSE, recursive = TRUE)
    plot_path <- sprintf("figures/pca_misty_sign_props/%s_%s_%s.pdf",
                         lesions, sign_type, view_name)
    layout <- "
    ##BBBB
    AABBBB
...
plot_misty_importances(lesions, 'int', 'pos')
plot_misty_importances(lesions, 'jux_5', 'pos')
plot_misty_importances(lesions, 'pra_15', 'pos')
