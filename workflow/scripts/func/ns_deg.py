...
def do_contrast(cdata, niche):
    cdata = cdata[cdata.obs['niches'] == niche].copy()
    genes = dc.filter_by_expr(cdata, group=None, min_count=10, min_total_count=15)
    print(niche, genes.size)
    ...
    try:
        ...
        results_df['contrast'] = 'CIvsCA'
        results_df['niche'] = niche
...
df = []
for niche in ['PPWM', 'LR', 'LC', 'VI']:
    tmp = do_contrast(pdata, niche).sort_values('padj', ascending=True)
    df.append(tmp)
df = pd.concat(df)
df.to_csv(out_path)
