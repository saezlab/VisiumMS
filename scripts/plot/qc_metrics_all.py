...
# python scripts/plot/qc_metrics_all.py --output cellbender
# python scripts/plot/qc_metrics_all.py --output cellranger
...
if args.output == "cellbender":
    input_dir = current_folder / ".." / ".." / "data" / "cellbender_qc_out"
    output_dir = current_folder / ".." / ".." / "out" / "cellbender_qc_out"
elif args.output == "cellranger":
    input_dir = current_folder / ".." / ".." / "data" / "cellranger_qc_out"
    output_dir = current_folder / ".." / ".." / "out" / "cellranger_qc_out"
else:
...
samples = [s.split(".")[0] for s in os.listdir(input_dir) if not s.startswith(".")]
for sample in samples:
    ...
    # Write to png
    fig.savefig(os.path.join(output_dir, 'qc_'+sample+'.png'))
...
# Save
fig.savefig(os.path.join(output_dir, 'qc_summary.png'))
